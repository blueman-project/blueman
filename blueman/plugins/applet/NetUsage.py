...
<interface>
  ...
  <object class="GtkDialog" id="dialog">
    ...
    <property name="title" translatable="yes">Traffic statistics</property>
...
class Dialog:
	...
	def __init__(self, parent):
		...
		self.parent = parent
		builder = gtk.Builder()
		builder.add_from_string(ui_def)
	...
	def update_time(self):
		...
		if time:
			...
			self.l_duration.props.label = _("%d %s %d %s and %d %s") % (delta.days, d, delta.seconds / 3600, h, delta.seconds % 3600 / 60, m)
		else:
	...
	def on_reset(self, button):
		d = gtk.MessageDialog(parent=self.dialog, flags=gtk.DIALOG_MODAL, type=gtk.MESSAGE_QUESTION, buttons=gtk.BUTTONS_YES_NO, message_format=_("Are you sure you want to reset the counter?"))
		res = d.run()
		d.destroy()	
...
class NetUsage(AppletPlugin, gobject.GObject):
	...
	__gsignals__ = {
		...
		#monitor, tx, rx
		'stats' : (gobject.SIGNAL_NO_HOOKS, gobject.TYPE_NONE, (gobject.TYPE_PYOBJECT, gobject.TYPE_PYOBJECT, gobject.TYPE_PYOBJECT,)),
	}
