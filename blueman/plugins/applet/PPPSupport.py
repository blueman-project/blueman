...
class Connection:
    def __init__(
        self,
        applet: "BluemanApplet",
        service: DialupNetwork,
        port: int,
        ok: Callable[[str], None],
        err: Callable[[GLib.Error], None],
    ):
    ...
    def connect(self) -> bool:
        c = Gio.Settings(
            schema_id="org.blueman.gsmsetting", path=f"/org/blueman/gsmsettings/{self.service.device['Address']}/"
        )

        m = Mechanism()
        m.PPPConnect(
            "(uss)", self.port, c["number"], c["apn"], result_handler=self.on_connected, error_handler=self.on_error
        )
...
class PPPSupport(AppletPlugin, RFCOMMConnectHandler):
    ...
    def rfcomm_connect_handler(
        ...
        err: Callable[[Union[RFCOMMError, GLib.Error]], None],
    ) -> bool:
